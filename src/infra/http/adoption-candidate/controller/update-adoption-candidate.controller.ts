...
import { updateAdoptionCandidateSchema, UpdateAdoptionCandidateInput } from '../schemas/update-adoption-candidate-schema';
...
import { UpdateAdoptionCandidateDocs } from '../docs/adoption-candidate.docs';
...
export class ControllerUpdateAdoptionCandidate {
  ...
  @Put(':id')
  @UpdateAdoptionCandidateDocs()
  async handle(
    @Param('id', new ZodValidationPipe(uuidParamSchema)) id: string,
    @Body(new ZodValidationPipe(updateAdoptionCandidateSchema)) body: UpdateAdoptionCandidateInput,
    @CurrentUser() actor: CurrentUserPayload,
  ) {
    const { name, phone, identityUrl } = body;

    const result = await this.updateCandidate.execute({ actor, id, name, phone, identityUrl });
