...
import { banAdoptionCandidateSchema, BanAdoptionCandidateInput } from '../schemas/ban-adoption-candidate-schema';
...
import { BanAdoptionCandidateDocs } from '../docs/adoption-candidate.docs';
...
export class ControllerBanAdoptionCandidate {
  ...
  @Patch(':id/ban')
  @Roles(Role.ADMIN)
  @BanAdoptionCandidateDocs()
  async handle(
    @Param('id', new ZodValidationPipe(uuidParamSchema)) id: string,
    @Body(new ZodValidationPipe(banAdoptionCandidateSchema)) body: BanAdoptionCandidateInput,
    @CurrentUser() actor: CurrentUserPayload,
  ) {
    const result = await this.banCandidate.execute({ actor, id, isBanned: body.isBanned, bannedReason: body.bannedReason });
